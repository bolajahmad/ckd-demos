...
function IssuedToken() {
  ...
  useEffect(() => {
    const updateFromInfo = async () => {
      const { lockScript, address } =
        await generateAccountFromPrivateKey(privKey);
  ...
  return (
    <>
      ...
      <ul>
        ...
        <li>Lock Script Hash: {lockScript?.hash()}</li>
      </ul>
      <br />
...
function ViewIssuedToken() {
  ...
  const onQuery = async () => {
    const cells = await queryIssuedTokenCells(xudtArgs as `0x{string}`).catch(
      alert,
    );
    ...
    <>
      ...
      <div>
        ...
        <input
          id="xudt-args"
          type="text"
          onChange={(e) => setXudtArgs(e.target.value)}
        />
      </div>
      ...
      {cells.length > 0 && (
        <h3>Result: all the cells which hosted this issued token</h3>
      )}
      {cells.map((cell, index) => (
        <div key={cell.outPoint.toBytes().toString() + index}>
          <p>Cell #{index}</p>
          <li>
            Token amount: {ccc.numLeFromBytes(cell.outputData).toString()}
          </li>
  ...
  return (
    <>
      ...
      <input
        id="udt"
        type="text"
        onChange={(e) => setUdtArgs(e.target.value)}
      />
      <br />
      ...
      <div>
        {tx && (
          <>
            <h4>Result</h4>
            <li>
              Transaction: <pre>{ccc.stringify(tx)}</pre>
            </li>
          </>
        )}
      </div>
